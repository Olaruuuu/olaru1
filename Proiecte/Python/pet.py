...
#walk towards right
  ...
  x -= -3
 window.geometry('100x100+'+str(x)+'+650')
 label.configure(image=frame)
 window.after(1,event,cycle,check,event_number,x)

window = tk.Tk()
#call buddy's action gif
idle = [tk.PhotoImage(file=impath+'Ninja.gif',format = 'gif -index %i' %(i)) for i in range(5)]#idle gif
...
window.wm_attributes('-transparentcolor','white', "-topmost",1)
label.pack()
#loop the program
